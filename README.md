...
We measure the purity of a pattern by comparing the probability of seeing a phrase in the topic- $\hat{t}$ collection
$D(t)$ and the probability of seeing it in any other topic- $\hat{t}$ collection
...
{ $d$ |topic $t$ is assigned to at least one word in $d$ }. $D(t, \hat{t})$ is the union of $D(t)$ and $D(\hat{t})$.
...
- `modules/purity.py` :
   ...
   Module that takes care of re-ranking a set of frequent patterns based on a score generated using their purity and
   support.
